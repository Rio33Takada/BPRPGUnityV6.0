...
public class BattleController : MonoBehaviour
{
    ...
    public void HandlePiecePlaced(CharacterPiece piece, Vector2Int origin)
    {
        ...
        foreach (var cell in GetPieceCellsWithFieldObject(piece))
            var enemy = cell.OccupiedObject?.GetParentEnemy();
                int damage = piece.character.CharacterData.attack * cell.CellInfo.power;

        foreach(var cell in GetPieceCellsWithFieldObject(piece))
            if (cell.OccupiedObject != null)

    private IEnumerable<(PieceCellInfo CellInfo, FieldObject OccupiedObject)> GetPieceCellsWithFieldObject(CharacterPiece piece)
    {
        foreach (var cell in piece.GetCellInfoCopy())
        {
            int posX = cell.offset.x + piece.PosX;
            int posY = cell.offset.y + piece.PosY;
            yield return (cell, fieldGrid.GetCell(posX, posY).OccupiedObject);
        }
    }
